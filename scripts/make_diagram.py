...
def box(x, y, w, h, text, fc, ec, fs=9, bold=False):
    ax.add_patch(
        FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.6,rounding_size=2", fc=fc, ec=ec, lw=1.6)
    )
    ax.text(
        x + w / 2,
        y + h / 2,
...
def arrow(x1, y1, x2, y2, color=INK):
    ax.add_patch(
        FancyArrowPatch(
            (x1, y1), (x2, y2), arrowstyle="-|>", mutation_scale=14, lw=1.5, color=color
        )
    )
...
ax.text(
    2.5,
    94.5,
    "OFFLINE PRECOMPUTE  ·  network OK  ·  no time budget",
    fontsize=10,
    color=OFFLINE_E,
    weight="bold",
)
...
ax.text(
    2.5,
    43.5,
    "RANK-TIME  ·  CPU only  ·  no network  ·  < 5 min  ·  numpy/pandas",
    fontsize=10,
    color=ONLINE_E,
    weight="bold",
)
...
arrow(23, 83.5, 28, 83.5)
arrow(47, 83.5, 52, 86)
arrow(23, 68.5, 28, 68.5)
arrow(47, 68.5, 52, 76)
arrow(47, 66, 52, 64)
arrow(72, 64, 76, 67)
...
arrow(41, 32.5, 43, 34.5)
arrow(41, 30, 43, 23)
arrow(51, 30, 61, 30)
arrow(51, 22.5, 60, 27)
arrow(76, 29.5, 78, 29.5)
arrow(87, 29.5, 88, 29.5)
arrow(93, 24, 86, 14)
