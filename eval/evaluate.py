...
def main():
    ...
    for name, m in results.items():
        print(
            f"  {name:30s} NDCG@10={m['NDCG@10']:.3f} NDCG@50={m['NDCG@50']:.3f} "
...
def _top100_composition(submission_path, candidates_path, rubric):
    ...
    try:
        top_ids = {
            r["candidate_id"] for r in _csv.DictReader(open(submission_path, encoding="utf-8"))
        }
        if not top_ids:
            return None
        titles = {}
        ...
        return (
            f"All **{ai_aligned}/{len(titles)}** of Lighthouse's top-100 hold an "
...
def _write_results(
    ...
    lines.append(
        "> **How to read this page.** The strongest evidence here is "
    ...
    lines.append(
        f"- Eval set for the metric tables: **{n} candidates** across 8 archetypes "
    ...
    lines.append(
        "These results stand without trusting our labels — they are facts about the "
        "ranking and the dataset's own planted traps.\n"
    )
    lines.append(
        "**Honeypot safety.** The dataset seeds ~80 subtly-impossible profiles, forced "
    ...
    if top100_fact:
        lines.append(
            f"**Top-100 composition.** {top100_fact} The provided `sample_submission` "
    ...
    if trap_table:
        lines.append(
            "**Trap resistance.** How far down each scorer pushes the dataset's traps "
        ...
        for grp, t in trap_table.items():
            lines.append(
                f"| {grp} | {t['n']} | **{t['med_full']:.0f}** | {t['med_anti']:.0f} | "
        ...
        if hpg:
            extra += (
                f"Remove the honeypot+gate stack and honeypots climb from median rank "
    ...
    lines.append(
        "> ⚠️ **Read this caption first.** The tiers below come from "
    ...
    lines.append(
        "Each row removes one piece and re-scores against the proxy labels. Single-"
    ...
    for name in [
        "– ablate role_coherence",
        "– ablate career_evidence",
        "– ablate trust_skills",
        "– no hard-negative gates",
        "– no honeypot filter",
        "– no behavioral modifier",
    ...
    if not _os.path.exists("eval/blind_results.md"):
        ...
        lines.append(
            "Because §2's labels are self-authored, the repo ships a **blind** "
        ...
        lines.append(
            "Results are written to `eval/blind_results.md` and summarised here once a "
...
def _row(name, m):
    return (
        f"| {name} | {m['NDCG@10']:.3f} | {m['NDCG@50']:.3f} | "
