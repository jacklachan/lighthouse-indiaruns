...
def _recs(scores):
    return [
        {"candidate_id": f"CAND_{i:07d}", "final_score": s, "honeypot": False}
...
def test_tie_break_candidate_id_ascending():
    ...
    recs = [
        {"candidate_id": cid, "final_score": 0.5, "honeypot": False}
...
def test_full_csv_passes_official_validator(tmp_path):
    ...
    scores = [
        round(random.choice([0.9, 0.8, 0.8, 0.5, 0.5, 0.3, random.random()]), 4) for _ in range(120)
    ]
    recs = _recs(scores)
    top = ranker.rank_records(recs, top=100)
    ...
    with open(out, "w", encoding="utf-8", newline="") as f:
        ...
        for r in top:
            w.writerow(
                [r["candidate_id"], r["rank"], f"{r['final_score']:.6f}", "grounded reason."]
            )
